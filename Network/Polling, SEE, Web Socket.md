...
클라이언트에서 고정된 시간 간격으로 서버에 request를 보냄
👇
서버는 request를에 대해 바로 response를 줌 
👇
...
클라이언트에서 request를 보냄
👇
서버는 요청에 대해 일정시간동안 대기 하다가 조건을 만족(이벤트발생)하면 response를 보냄
👇
클라이언트에서 request를 보냄
...
클라이언트에서 request(text/event-stream)를 보냄
👇
서버에서 일방적으로 데이터들을 보냄
👇
서버에서 연결을 종료함

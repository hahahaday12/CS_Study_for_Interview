...
1. url 에 입력된 값을 브라우저 내부에서 결정된 규칙에 따라 그 의미를 조사한다.
   ...
   - 브라우저는 url을 분석하여 요청 대상의 호스트 이름, 요청 대상의 경로, 쿼리 문자열을 추출한다. 호스트 이름은 웹 서버의 IP 주소를 식별하는 데 되고, 경로는 웹 페이지의 위치를 식별하는 데 사용된다. 쿼리 문자열은 웹 서버에 전송되는 추가 정보를 포함하는데, 쿼리 문자열의 각 항목은 name=value 형식으로 지정된다. name은 쿼리 문자열의 이름이고 value는 쿼리 문자열의 값이다

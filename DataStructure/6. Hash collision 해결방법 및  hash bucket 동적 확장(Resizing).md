...
- 그러면 그냥 '010-1111-1111'을 index로 사용하면 안되는건가?
  ...
  - 해시 함수를 사용하여 키를 고정된 크기의 해시 값으로 변환하면, 상대적으로 작은 크기 index(hash value)를 얻을 수 있다.
...
- put('Sandra Dee')
  ...
  - 일치하지 않으므로 해당 버킷에 새로운 버킷공간을 가리키는 주소를 저장하고 가리키는 버킷에는 [ 'Sandra Dee', '521-9655' ]를 저장한다.
  - 여기에서 가장 중요한 점은 `인접한 버킷 공간`이 아닌 `완전히 새로운 버킷 공간`에 저장하고 해당 버킷의 주소를 저장한다는 점
- delete('John Smith')
